...
When playtesting ordinary turns, `scripts/validate_state.py` can also check optional `next_affordances` and `last_intent` on the ledger. Use these for debug QA of action-entry hooks and natural-language intent preservation; do not expose the structured objects to the player unless they ask for raw state.
